...
import { Surface, Text, Card, Avatar, Chip, Divider, IconButton, Button, ActivityIndicator } from "react-native-paper";
...
export default function Sub() {
  ...
  // 加载关注的帖子数据
  const loadFollowingPosts = useCallback(async (pageNum = 1, refresh = false) => {
    if (!username) {
      setLoading(false);
      setRefreshing(false);
      return;
    ...
    try {
      ...
      const response = await request({
        method: 'GET',
        url: `/post/following?username=${username}&page=${pageNum}&size=10`,
      });

      if (response.data.code === 200) {
        const newPosts = response.data.data || [];
        console.log(`加载了${newPosts.length}条关注的帖子`);
        ...
        console.error('获取关注帖子失败:', response.data.msg);
      }
    } catch (error) {
      console.error('加载关注帖子数据出错:', error);
    } finally {
      setRefreshing(false);
      setLoading(false);
    }
  ...
  }, [refreshing, loading, hasMore, page, loadFollowingPosts]);
  ...
  const handleFollow = async (followingUsername: string, event: any) => {
    ...
    try {
      ...
      if (response.data?.code !== 200) {
        ...
        // 操作成功提示
        if (!isFollowed) {
          // Alert.alert('关注成功', `你已成功关注 ${followingUsername}`);
        }
      }
    } catch (error) {
  ...
  const renderEmpty = () => {
    ...
    if (!isLoggedIn) {
      return (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>请先登录查看关注内容</Text>
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <TouchableOpacity onPress={() => router.push('/(tabs)/home/hot')}>
          <Text style={[styles.createPostText, { color: theme.colors.primary }]}>
            寻找宝藏博主
          </Text>
        </TouchableOpacity>
